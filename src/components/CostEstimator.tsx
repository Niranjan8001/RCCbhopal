    ...
    <section ref={sectionRef} className="section-padding relative" id="estimator">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-2xl mx-auto mt-8">
          <div className="glass-card-premium p-8 relative overflow-hidden border border-accent-green/10">
            {/* Background glow */}
            <div className="absolute top-0 right-0 w-48 h-48 rounded-full blur-[80px] opacity-10 bg-accent-green pointer-events-none" />

            {/* Title row */}
            <div className="flex items-start gap-4 mb-5 relative z-10">
              <div className="p-2.5 rounded-xl bg-accent-green/10 text-accent-green flex-shrink-0 mt-0.5">
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                </svg>
              </div>
              <h3 className="text-lg font-black text-white leading-snug">The RCC Financial Transparency Guarantee</h3>
            </div>

            <p className="text-sm text-muted leading-relaxed mb-6 relative z-10">
              We eliminate the biggest fear in construction: cost overruns. You get a guaranteed week-by-week timeline and a transparent payment schedule tied directly to construction milestones. No hidden fees. No sudden material price hikes passed blindly to you.
            </p>

            {/* Three pillars */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 relative z-10">
              {[
                { label: 'Week-by-Week Timeline', icon: 'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5' },
                { label: 'Milestone-Linked Payments', icon: 'M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z' },
                { label: 'Zero Surprise Charges', icon: 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
              ].map((item) => (
                <div key={item.label} className="flex items-center gap-3 p-3 rounded-xl bg-accent-green/5 border border-accent-green/10">
                  <div className="text-accent-green flex-shrink-0">
                    <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d={item.icon} />
                    </svg>
                  </div>
                  <span className="text-xs font-semibold text-foreground/80 leading-tight">{item.label}</span>
                </div>
              ))}
            </div>
          </div>
